...
operatorRouter.get('/', asyncRoute(async (_req, res) => {
  ...
  const result = await pool.query(
    `SELECT u.id,u.name,u.email,m.active,m.created_at
     FROM tenant_memberships m JOIN users u ON u.id=m.actor_id
     WHERE m.user_id=$1 AND m.role='OPERATOR' ORDER BY m.created_at DESC`, [tenantId]);
...
operatorRouter.post('/', asyncRoute(async (req, res) => {
  ...
  try {
    ...
    await client.query(`INSERT INTO tenant_memberships(actor_id,user_id,role) VALUES($1,$2,'OPERATOR')`, [id, tenantId]);
    await client.query(`INSERT INTO admin_audit_logs(user_id,actor_id,actor_role,target_user_id,action,details)
...
operatorRouter.patch('/:id/status', asyncRoute(async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const result = await client.query(`UPDATE tenant_memberships SET active=$3
      WHERE user_id=$1 AND actor_id=$2 AND role='OPERATOR' RETURNING actor_id`, [tenantId, operatorId, active]);
    ...
    if (!active) {
      ...
      await client.query('DELETE FROM auth_sessions WHERE actor_id=$1 AND user_id=$2', [operatorId, tenantId]);
    }
    await client.query(`INSERT INTO admin_audit_logs(user_id,actor_id,actor_role,target_user_id,action,details)
