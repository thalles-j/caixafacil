...
- As novas colunas pertencem a `customers`, cuja RLS forçada continua vigente. Consultas usam `user_id` autenticado e UUID da entidade; IDs enviados pelo cliente não selecionam outro tenant. Operador pode registrar/revogar consentimento e preparar cobrança, mas não anonimizar.
